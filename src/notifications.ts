...
export class NotificationManager extends WorkerEntrypoint {
     ...
     */
    notify_undeliverable = async (parent: EmailData, to_alias_address: string) => {
        await this.#try_notification_with_rate_limit(
     ...
     */
    notify_new_user = async (user: AliasRow, links: { verification_link: URL; deletion_link: URL }) => {
        await this.#try_notification_with_rate_limit(
            send_email(this.env, {
     ...
     */
    notify_rate_limited = async (parent: EmailData) => {
        await this.#try_notification_with_rate_limit(
            send_reply(this.env, parent, {
                from: this.system_address,
                from_name: "ArpaMail",
     ...
     */
    notify_reply_unavailable = async (parent: EmailData) => {
        await this.#try_notification_with_rate_limit(
            send_reply(this.env, parent, {
                from: this.system_address,
                from_name: "ArpaMail",
     ...
     */
    #try_notification_with_rate_limit = async (
     ...
     */
    #rate_limit_notification = async (key: string) => {
